(() => {
  ...
  function renderManager(){
    ...
    <section class="panel full"><h2>Automatic program roster</h2><p class="helper">Eligible Stripe memberships appear automatically, regardless of usual weekday. Calendar records can supply missing student/program details and highlight expected arrivals. Calendar entries alone never override invalid billing status. Correct student names, program and parent emails in Stripe; use Allowed only for a staff attendance hold. A hold does not change billing.</p><div id="roster-rows"></div><div class="actions"><button class="primary" data-action="save-roster">Save attendance holds</button></div><p class="helper">Cancelled, paused, unpaid, past-due, not-started, or ambiguous memberships are not automatically admitted. ${manager.issues?.length||0} membership records across the account need review or are ineligible.</p></section></div>`;
    renderRoster();
  ...
  app.addEventListener("click",async ev=>{
    ...
    try {
      ...
      if(action==="export"){
        ...
        const rows=[["Class date","Class","Student","Status","Arrival (New York)"],...attendanceStudents().map(s=>{const a=manager.attendance.find(a=>a.studentId===s.id&&a.present);return[date,cls.title,s.name,a?"Present":"Not checked in",a?timeLabel(a.at):""];})];
